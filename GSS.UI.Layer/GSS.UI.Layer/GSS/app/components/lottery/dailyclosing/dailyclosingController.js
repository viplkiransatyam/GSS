angular.module('GasApp').controller("DailyClosingController", ['$scope', '$rootScope', '$filter', 'lotteryService', function ($scope, $rootScope, $filter, lotteryService) {   
    ...
    $scope.addBook = function () {
        ...
        if (!angular.isUndefined($scope.dailyClosing.gameNumber_1) && $scope.dailyClosing.gameNumber_1 != "") {
            fullGame = $scope.dailyClosing.gameNumber_1;
            digit = 3;
        }
        else if (!angular.isUndefined($scope.dailyClosing.gameNumber_2) && $scope.dailyClosing.gameNumber_2 != "") {
            fullGame = $scope.dailyClosing.gameNumber_2;
            digit = 4;
        }
        ...
        $scope.booksactive = true;
        if (angular.isUndefined($scope.dailyClosing.gameNumber_1) && angular.isUndefined($scope.dailyClosing.gameNumber_2)) {
            sweetAlert("Error!!", "Please Enter Book Serial No.", "error");
        }else {
            var LastTicketNo = [];
            var postData = {};
            postData.StoreID = storeID;
            //LastTicketNo = fullGame.split(/[.\-_]/);
            ...
            if (digit == 3) {
                ...
                LastTicketNo[0] = fullGame.substring(0, 4);
                LastTicketNo[1] = fullGame.substring(4, 10);
                LastTicketNo[2] = fullGame.substring(10, 13);
            }
                ...
                if(hasAddObject){
                    ...
                    lotteryService.getClosingTickets(postData).success(function (response) {
                        ...
                        if(LastTicketNo[2]<=response.NoOfTickets){
                            ...
                            $scope.dailyClosing.gameNumber_1 = "";
                            $scope.dailyClosing.gameNumber_2 = "";
                        }
                        $scope.loading = false;
                    }).error(function (response) {
                        sweetAlert("Error!!", response, "error");
