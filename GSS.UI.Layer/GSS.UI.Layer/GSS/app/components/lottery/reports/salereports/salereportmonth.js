...
angular.module('GasApp').controller("SaleReportMonthController", ['$scope', '$rootScope', '$filter', 'lotteryService', 'NgTableParams', 'Excel', '$timeout', 'moment', 'calendarConfig', function ($scope, $rootScope, $filter, lotteryService, NgTableParams, Excel, $timeout, moment, calendarConfig) {
    ...
    $scope.getLotterySalesMonth = function(){
         var postData = {};
            ...
            for (var i = 0; i < response.length; i++) {              
                $scope.events.push({instantsale:response[i].InstantSale,
                                    onlinesale:response[i].OnlineSale,
                                    InstantCommission:response[i].InstantCommission,
                                    OnlineCommission:response[i].OnlineCommission,
                                    CashCommission:response[i].CashCommission,
                                    startsAt:$filter('date')(response[i].Date, 'dd-MMM-yyyy')
                                });
            }
        ...
        lotteryService.getLotteryDetailedSalesReport(postData).success(function (response) {
            ...
            if (response.length <= 0) {
                ...
                $scope.tableParams = new NgTableParams({
                    sorting: { Date: "asc", ShiftCode: "asc", Game: "asc", Pack: "asc", GameName: "asc", TicketValue: "asc", BoxNo: "asc", StartTicket: "asc", EndTicket: "asc", NoOfTicketSold: "asc", AmountSold: "asc", PackStatus: "asc" }
                }, {
