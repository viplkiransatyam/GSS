angular.module('GasApp').controller("ReceivebooksController", ['$scope', '$rootScope', '$filter', 'lotteryService', function ($scope, $rootScope, $filter, lotteryService) {   
    ...
    $scope.addBook = function () {
        ...
        if (!angular.isUndefined($scope.gameNumber_1) && $scope.gameNumber_1 != "") {
            fullGame = $scope.gameNumber_1;
            digit = 3;
        }
        else if (!angular.isUndefined($scope.gameNumber_2) && $scope.gameNumber_2 != "") {
            fullGame = $scope.gameNumber_2;
            digit = 4;
        }

        if (angular.isUndefined(fullGame) || fullGame == "") {
            return;
